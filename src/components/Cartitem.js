...
import { decrementQuantity, deleteItem, incrementQuantity, resetCart } from '../redux/Sale';
...
const Cartitem = () => {
  ...
  return (
    ...
    <div>
      {
        productData.map((item)=>(
          ...
          >
            ...
            <h2 className='w-5 font-semibold'>{item.title}</h2>
    ...
    <button
      ...
      <Link to='/'><button className='mt-8 ml-7 flex items-center gap-1 text-gray-400 hover:text-black duration-300 font-semibold'><span><HiOutlineArrowLeft/></span>Go Shopping</button></Link>
    <ToastContainer
        position='top-left'
        autoClose={2000}
